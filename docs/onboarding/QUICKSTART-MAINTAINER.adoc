...
// Replace rsr-template-repo, kitchenspeak, {{DEPS}} with actuals
...
| `$PREFIX/share/kitchenspeak/`
| Data files, assets

| `$PREFIX/share/doc/kitchenspeak/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/kitchenspeak/config.toml`

Fallback: `$HOME/.config/kitchenspeak/config.toml`
...
Or via OPSM: `opsm update kitchenspeak`
...
[source,bash]
----
just install --prefix=/opt/kitchenspeak-instance1 --config=/etc/kitchenspeak/instance1.toml
just install --prefix=/opt/kitchenspeak-instance2 --config=/etc/kitchenspeak/instance2.toml
----
